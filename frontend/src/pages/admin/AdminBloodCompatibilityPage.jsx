...
// import BloodCompatibilityFormModal from '../../components/admin/BloodCompatibilityFormModal'; // Component này cần được tạo
...
const AdminBloodCompatibilityPage = () => {
    ...
    const handlePageChange = (newPage) => {
        ...
        // fetchRules(newPage, pageSize); // fetchRules sẽ tự lấy currentPage mới
    };
    ...
    const handleDelete = async (id) => {
        if (window.confirm(`Bạn có chắc chắn muốn xóa quy tắc tương thích (ID: ${id}) không?`)) {
            ...
            try {
                ...
                fetchRules(); // Tải lại trang hiện tại hoặc trang đầu
            } catch (error) {
        ...
        <div className="p-6">
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex items-center space-x-2">
                    <Button onClick={() => fetchRules(currentPage, pageSize)} variant="secondary" className="p-2" title="Làm mới" disabled={isLoading}>
            ...
            {isLoading && !rulesPage ? (
                ...
                <>
                    <div className="bg-white shadow-md rounded-lg overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    {['ID', 'Người cho', 'Người nhận', 'Thành phần', 'Tương thích', 'Khẩn cấp', 'Điểm', 'Hành động'].map(header => (
                                ...
                                {rulesPage.content.map((rule) => ( //
                                    <tr key={rule.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-4 py-4 whitespace-nowrap text-sm text-center">
                                            {rule.isCompatible ? <CheckCircle className="text-green-500" size={20} /> : <XCircle className="text-red-500" size={20} />}
                                        </td>
                                        <td className="px-4 py-4 whitespace-nowrap text-sm text-center">
                                            {rule.isEmergencyCompatible ? <CheckCircle className="text-orange-500" size={20} /> : <XCircle className="text-gray-400" size={20} />}
                                        </td>
                    ...
                    {/* Pagination Controls - tương tự AdminUserListPage */}
                    {rulesPage.totalPages > 1 && (
                        <div className="mt-6 flex flex-col sm:flex-row justify-between items-center">
                            ...
                            {/* Thêm nút chuyển trang ở đây nếu cần */}
                        </div>
                    )}
                </>
            ) : (
